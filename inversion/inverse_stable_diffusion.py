...
class InversableStableDiffusionPipeline(ModifiedStableDiffusionPipeline):
    ...
    def backward_diffusion(
        ...
        reverse_scheduler=None,
        **kwargs,
    ):
        """ Generate image from text prompt and latents
        """
        if reverse_scheduler is not None:
            reverse_scheduler = reverse_scheduler
        else:
            reverse_scheduler = self.scheduler
        ...
        # set timesteps
        reverse_scheduler.set_timesteps(num_inference_steps)
        # Some schedulers like PNDM have timesteps as arrays
        # It's more optimized to move all timesteps to correct device beforehand
        timesteps_tensor = reverse_scheduler.timesteps.to(self.device)
        # scale the initial noise by the standard deviation required by the scheduler
        latents = latents * reverse_scheduler.init_noise_sigma
        ...
        for i, t in enumerate(self.progress_bar(timesteps_tensor if not reverse_process else reversed(timesteps_tensor))):
            ...
            latent_model_input = reverse_scheduler.scale_model_input(latent_model_input, t)
            ...
            prev_timestep = (
                t
                - reverse_scheduler.config.num_train_timesteps
                // reverse_scheduler.num_inference_steps
            )
            ...
            # ddim
            alpha_prod_t = reverse_scheduler.alphas_cumprod[t]
            alpha_prod_t_prev = (
                reverse_scheduler.alphas_cumprod[prev_timestep]
                if prev_timestep >= 0
                # else reverse_scheduler.final_alpha_cumprod
                else reverse_scheduler.alphas_cumprod[0]
            )
            if reverse_process:
