...
class Gaussian_Shading:
    ...
    def truncSampling(self, message):
        m = message.flatten() * 2 - 1.0
        z = torch.randn(self.latentlength).abs()
        z = m * z
        z = z.reshape(1, 4, 64, 64).half()
        return z.cuda()


    def create_watermark_and_return_w_IDENTITY(self):
        self.watermark = torch.randint(0, 2, [1, 4 // self.ch, 64 // self.hw, 64 // self.hw]).cuda()
        self.key = torch.randint(0, 1, [1, 4, 64, 64]).cuda()
        sd = self.watermark.repeat(1,self.ch,self.hw,self.hw)
        m = ((sd + self.key) % 2).flatten().cpu()
        w = self.truncSampling(m)
        return w
    
    def create_watermark_and_return_w_XOR(self):
        self.watermark = torch.randint(0, 2, [1, 4 // self.ch, 64 // self.hw, 64 // self.hw]).cuda()
